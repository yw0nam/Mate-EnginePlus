...
Assets/MATE ENGINE - Scripts/
  Hermes/                     # Backend integration (9 files)
    ...
    EmotionMapper.cs          # YAML → keyframes
    ...
    Keyframe.cs               # Animation keyframe data class
  DesktopMatePlus/            # UI + session management (14 files)
...
MATE ENGINE - Resources/      # YAML configs (emotion_motion_map.yaml)
...
- Tracks `LastResponseId` for multi-turn continuity
...
- Composes: chunker, preprocessor, emotion mapper, TTS queue, audio player
...
**EmotionMapper** (`Hermes/EmotionMapper.cs`)
- Loads YAML from `Assets/MATE ENGINE - Resources/emotion_motion_map.yaml`
- Maps emotion string → `List<Keyframe>`
- Fallback: hardcoded neutral keyframe if YAML missing or emotion unknown
...
- `Enqueue(sequence, text, emotion, keyframes, refId)` creates synthesis task
- `WaitBarrierAsync(timeout=30s)` awaits all pending, cancels stragglers on timeout
- Emits results in sequence order via `OnResult(seq, wav, emotion, keyframes)` callback
...
- Fires `OnWavChunkStarted(int sequence, string emotion, List<Keyframe> keyframes)` event
...
- Animates blendshapes per emotion + keyframes
- Crossfades between emotions smoothly
...
**Check Irodori-TTS**:
```powershell
Invoke-WebRequest http://localhost:8091/health -UseBasicParsing
```
...
- **Voice catalog UI**: Currently Inspector-only. Mid-session voice swap not yet supported.
- **KeyframeAnimationBridge**: Keyframes from `EmotionMapper` flow through `OnWavChunkStarted` but are currently unused.
